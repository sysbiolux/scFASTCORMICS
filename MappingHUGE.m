function[input_data]=MappingHUGE(input_data, model_composite, path, printLevel, scdataset)
%% Mapping the genes from the sc-RNA seq data to the composite model
if printLevel==1
    %Creates a folder named Mapped
    mkdir ([path,'\MappedHUGE']);
end
...
for i=1:files_length
    ...
    name = [path,'\Mapped\MappedHUGE_',scdataset(i).name];
