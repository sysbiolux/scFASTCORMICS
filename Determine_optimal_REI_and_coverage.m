function[best]=Determine_optimal_REI_and_coverage(Expanded_input_model,path, T, plot_formula, Cover_range, REI_range, Optimization_global)
...
% Number of reactions which should or not be there in the expanded input model (BE int N & BU int N reactions)

Expanded_input_model_reactions = (cellfun(@(S) S(1:end-2), Expanded_input_model.rxns, 'Uniform', 0));



% Core genes/reactions (C)

% % Reactions (bulk data) intersect with multi-cell population A
Reactions_Should_be = T.Table_Reaction_Expressed;

% Genes/Reactions in multi-cell population model (A)
A = sum(Optimization_global(1).A,1)';
C = sum(Optimization_global(1).C_keep,1)';

BE_inter_N=numel(intersect(Expanded_input_model_reactions,T(1).Rxns_Present));
BU_inter_N=numel(intersect(Expanded_input_model_reactions,T(1).Rxns_Not_Present));
